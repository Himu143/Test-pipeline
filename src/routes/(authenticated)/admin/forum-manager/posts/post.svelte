<script lang="ts">
	...
	import { ChevronRight, CircleCheck, Trash2 } from 'lucide-svelte';
...
>
	...
	<div class="flex flex-wrap items-start justify-between gap-3 px-4 pt-4 pb-3 sm:px-5">
		...
		<div class="flex flex-shrink-0 flex-wrap items-center gap-2">
			{#if type == 'pending'}
				<Button
					size="sm"
					class="h-8 rounded-full bg-red-500 px-3 text-xs font-semibold text-white hover:bg-red-600 sm:px-4"
					onclick={async () => {
						rejectLoading = true;
						await forumManagerPostStore.reject(post.id);
						rejectLoading = false;
					}}
			...
			<Button
				size="sm"
				class="h-8 rounded-full bg-green-500 px-3 text-xs font-semibold text-white hover:bg-green-600 sm:px-4"
				onclick={async () => {
					approveLoading = true;
					await forumManagerPostStore.approve(post.id);
	...
	<div class="px-4 pb-2 sm:px-5">
		...
		{#if post.header}
			<h3 class="mt-1.5 text-sm leading-snug font-bold text-gray-900 sm:text-base">
				{post.header}
			</h3>
		{/if}

		<p class="mt-1.5 line-clamp-3 text-sm leading-relaxed text-gray-600">{post.content}</p>
	</div>

	<!-- Footer: View Details -->
	<div class="flex items-center justify-end px-4 py-2.5 sm:px-5">
		<Button type="button" variant="secondary" onclick={() => (detailOpen = true)}>
			View Details
...
<PostDetailsModal bind:open={detailOpen} postId={post.id} />
